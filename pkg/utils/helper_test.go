...
func TestCanInjectEni_CombinedSelectors(t *testing.T) {
	...
	sgs := canGetSecurityGroups(sgpList, testPod, testSA, nil)
	assert.True(t, isEverySecurityGroupIncluded(sgs))
}

// TestCanInjectEni_PodSelectors tests SGP with Pod selector.
...
func TestCanInjectEni_MismatchedSASelector(t *testing.T) {
	...
	sgs := canGetSecurityGroups(sgpList, testPod, mismatchedSa, nil)
